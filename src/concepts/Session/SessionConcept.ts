...
export default class SessionConcept {
   ...
   */
  async create({ user }: { user: User }): Promise<{ session: Session; token: string }> {
    ...
    console.log(`✅ Created session for user ${user}, expires at ${expiresAt.toISOString()}`);
   ...
   */
  async end({ session }: { session: Session }): Promise<Empty | { error: string }> {
   ...
   */
  async endByToken({ token }: { token: string }): Promise<Empty | { error: string }> {
    const result = await this.sessions.updateOne(
      { token },
      { $set: { isActive: false } }
    );
  ...
  > {
    ...
    if (!isValid) {
      console.log(`⚠️ Invalid session: ${sessionDoc.isActive ? 'expired' : 'inactive'}`);
    }
