...
import OutputRenderConcept from "../Rendering/Rendering.ts";
...
Deno.test("OutputRenderConcept - render", async (t) => {
  await t.step("should create and return a new OutputVersion", async () => {
    ...
    assertEquals(
      result.output.renderedData.textElements[0].text,
      "Hello, World!",
    );
    assertEquals(
...
Deno.test("OutputRenderConcept - Queries", async (t) => {
  await t.step(
    "_getOutputVersionById should return the correct output",
    async () => {
      await clearCollection();
  ...
  await t.step(
    "_getOutputVersionById should return an empty array if not found",
    async () => {
      await clearCollection();
  ...
  await t.step(
    "_getAllOutputVersions should return an empty array if no outputs exist",
    async () => {
      await clearCollection();
