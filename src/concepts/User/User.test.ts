...
Deno.test("UserConcept: Operational Principle", async () => {
  ...
  if ("error" in createResult) {
    ...
    assertEquals(
      typeof createResult.user,
      "string",
      "create should return a user ID",
    );
    ...
    assertEquals(
      getUserByIdResult.length,
      1,
      "_getUserById should return one user",
    );
    assertEquals(
      getUserByIdResult[0].user,
      userId,
      "Returned user ID should match input",
    );

    const getUsernameResult = await userConcept._getUserUsername({
      user: userId,
    });
    console.log("Query: _getUserUsername", { user: userId });
    console.log("Output:", getUsernameResult);
    assertEquals(
      getUsernameResult.length,
      1,
      "_getUserUsername should return one username",
    );
    assertEquals(
      getUsernameResult[0].username,
    ...
    assertEquals(
      getEmailResult.length,
      1,
      "_getUserEmail should return one email",
    );
    assertEquals(
      getEmailResult[0].email,
      createUserArgs.email,
      "Email should match",
    );

    const getProfilePicResult = await userConcept._getUserProfilePic({
      user: userId,
    });
    console.log("Query: _getUserProfilePic", { user: userId });
    console.log("Output:", getProfilePicResult);
    assertEquals(
      getProfilePicResult.length,
      1,
      "_getUserProfilePic should return one profile pic",
    );
    assertEquals(
      getProfilePicResult[0].profilePic,
      createUserArgs.profilePic,
      "Profile pic should match",
    );
    ...
    const changeProfilePicResult = await userConcept.changeProfilePic(
      changeProfilePicArgs,
    );
    console.log("Output:", changeProfilePicResult);
    assertEquals(
      changeProfilePicResult,
      {},
      "changeProfilePic should return an empty object on success",
    );

    // 4. Verify profile picture change
    const updatedProfilePicResult = await userConcept._getUserProfilePic({
      user: userId,
    });
    console.log("Query: _getUserProfilePic after change", { user: userId });
    console.log("Output:", updatedProfilePicResult);
    assertEquals(
      updatedProfilePicResult.length,
      1,
      "Profile pic query after change should return one result",
    );
    assertEquals(
      updatedProfilePicResult[0].profilePic,
      newProfilePic,
      "Profile pic should be updated",
    );
    ...
    assertEquals(
      deleteResult,
      {},
      "delete should return an empty object on success",
    );
    ...
    assertEquals(
      verifyDeleteResult.length,
      0,
      "_getUserById should return no user after deletion",
    );
  }
  await client.close();
});
...
Deno.test("UserConcept: Interesting Scenarios", async () => {
  ...
  console.log(
    ...
    invalidPasswordFormatArgs,
  );
  const invalidPasswordFormatResult = await userConcept.create(
    invalidPasswordFormatArgs,
  );
  console.log("Output:", invalidPasswordFormatResult);
  if ("error" in invalidPasswordFormatResult) {
    assertEquals(
      invalidPasswordFormatResult.error,
      "Username and password must consist solely of alphabets, numbers, underscores, and hyphens.",
      "create should return error for invalid password format",
    );
  } else assertFalse;
  ...
  const deleteNonExistentResult = await userConcept.delete(
    deleteNonExistentArgs,
  );
  console.log("Output:", deleteNonExistentResult);
  assertEquals(
    deleteNonExistentResult.error,
    "User not found.",
    "delete should return error for non-existent user",
  );

  // Scenario 3: Change profile picture of a non-existent user
  const changePicNonExistentArgs = {
    user: nonExistentUserId,
    newProfilePic: "http://example.com/fake.jpg",
  };
  console.log(
    "Action: changeProfilePic (non-existent user)",
    changePicNonExistentArgs,
  );
  const changePicNonExistentResult = await userConcept.changeProfilePic(
    changePicNonExistentArgs,
  );
  console.log("Output:", changePicNonExistentResult);
  assertEquals(
    changePicNonExistentResult.error,
    "User not found.",
    "changeProfilePic should return error for non-existent user",
  );
  await client.close();
});
