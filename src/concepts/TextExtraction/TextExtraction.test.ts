...
Deno.test("TextExtractionConcept: extractTextFromMedia", async (t) => {
  ...
  const textExtraction = new TextExtractionConcept(db, llm);

  await t.step(
    "should extract text and create an extraction result",
    async () => {
      const imagePath = "LionKing.jpg" as ID;
      ...
      const locations = await textExtraction.locations
        .find({ extractionResultId: result.results })
        .toArray();
  ...
  // await t.step(
  //   "should handle multiple extractions for the same image",
  //   async () => {
  //     const imagePath = "path/to/another_image.jpg" as ID;
  //     await textExtraction.extractTextFromMedia({ image: imagePath });
  //     const result2 = await textExtraction.extractTextFromMedia({
  //       image: imagePath,
  //     });
...
//   await t.step(
//     "should return an error if extraction result does not exist",
//     async () => {
...
//   await t.step(
//     "should return an error if coordinates are negative",
//     async () => {
...
//       throw new Error(
//         "Result is not of expected type { result: ExtractionResult }",
//       );
//     }
...
//     assertEquals(extractionResults.length, 1);
//     assertEquals(locations.length, 1);
//     assertEquals(extractionResults[0]._id, result.result);
//     assertEquals(extractionResults[0].imagePath, imagePath);
//     assertEquals(extractionResults[0].extractedText, "");
//     assertEquals(extractionResults[0].textId, `${imagePath}_0`);
//     assertEquals(locations[0].fromCoord, fromCoord);
//     assertEquals(locations[0].toCoord, toCoord);
//   });
...
//   const addedExtraction = await textExtraction.extractionResults.findOne({
//     textId: textIdToDelete,
//     imagePath: imagePath,
//   });
...
//   await t.step(
//     "should return an error if extraction does not exist",
//     async () => {
//       const deleteResult = await textExtraction.deleteExtraction({
...
//   await t.step(
//     "Principle: Given an image, AI extracts text and produces a transcript with metadata.",
//     async () => {
...
//       assertEquals(
//         createdExtraction?.extractedText,
//         `Placeholder extracted text for ${imagePath}`,
//       );
...
//       // Simulating user editing and adding more text
...
//       // Type guard to ensure result has the expected structure
//       if (
//         typeof secondExtractionResult !== "object" ||
//         !("result" in secondExtractionResult)
//       ) {
//         throw new Error(
//           "Result is not of expected type { result: ExtractionResult }",
//         );
...
//       const secondCreatedExtraction = await textExtraction.extractionResults
//         .findOne({
...
//   await t.step(
//     "editExtractText should return error for non-existent ID",
//     async () => {
...
//   await t.step(
//     "editLocation should return error for non-existent ID",
//     async () => {
...
//   await t.step(
//     "deleteExtraction should return error for non-existent textId/imagePath",
//     async () => {
