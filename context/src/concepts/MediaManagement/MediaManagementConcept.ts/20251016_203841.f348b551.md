---
timestamp: 'Thu Oct 16 2025 20:38:41 GMT-0400 (Eastern Daylight Time)'
content_id: f348b55103a5be12bdc7a9fba357ebb9b6c779e229b91c968356d6a8b893827e
---
...
export default class MediaManagementConcept {
  ...
  /**
   * uploadMedia (owner: User, filePath: string, fileName: string, fileType: string): (mediaItem: MediaItem)
   *
   ...
   *
   * **requires**: `owner` is a valid User ID. `filePath`, `fileName`, `fileType` are non-empty strings.
   *
   * **effects**: Creates a new MediaItem `m`; sets `m.owner` to `owner`; sets `m.filePath` to `filePath`; sets `m.fileName` to `fileName`; sets `m.fileType` to `fileType`; sets `m.uploadTimestamp` to the current time; sets `m.accessControl` to "private" by default; returns `m._id` as `mediaItem`.
   */
  async uploadMedia({
    owner,filePath,fileName,fileType,}: {owner: User;filePath: string;fileName: string;fileType: string;})
...
[^1:] App storage is within the user's local storage. The cloudfilePath doesn't change.
   */
