...
content_id: c86b6e4daf05fcad892c9ff5a0681bf31aff3a68f561d51c2fc02018f054407d
---
...
interface Users {
  _id: User;
  username: string;
  passwordHash: string; // Renamed from password to avoid confusion with plain password
  profilePic: Image;
  email: string;
...
export default class UserConcept {
  ...
  async create({
    ...
    const passwordHash = await encryptPassword(password); // Assuming encryptPassword handles hashing
    const newUser: Users = {
      _id: freshID(),
      username: username,
   ...
   */
  async delete({ user }: { user: User }): Promise<Empty> {
